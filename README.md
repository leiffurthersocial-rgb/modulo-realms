...
Run `npm run check:aegean` for the serial expansion and original-game compatibility checks.
...
src/
  ...
  game/
    ...
    art/              every pixel in the game is drawn here, at runtime
      ...
      uiArt.ts        UI frames, buttons, slots, glyph icons, crest and logo (9-slice pixel art)
      uiCanvas.ts     pixel-font text, frames and prompts drawn onto a canvas
      mapArt.ts       the parchment atlas, its fog of war, and dungeon plans
    world/
  ...
  ui/                 React panels (HUD, inventory, map, journal, shop, forge, travel, ...)
    kit/              the UI design system: palette tokens, the pixel fonts (built as
                      TrueType at boot), Modal/Button/Slider/Tabs/SegBar/..., sound hooks
    hud/              vitals, hotbar, minimap, banners
  styles/global.css   the UI stylesheet, written in UI pixels on tokens and generated frames
```
...
- **Simulation is not React.** The game loop mutates plain objects and draws to a canvas.
  ...
  that the game bumps when something UI-visible changes. Each HUD piece polls the game on
  its own small interval and only re-renders when what it shows has changed.
- **The UI is pixel art too.** The overlay and the world camera are both scaled by whole
  numbers only; fonts, frames and icons are generated in code like every other sprite.
  See `docs/ui-overhaul/README.md` and `scripts/check-ui-style.ts`.
...
existing game. That includes the interface: the two pixel fonts are drawn glyph by glyph in
`src/ui/kit/glyphs.ts` and assembled into font files at runtime, so there are no font files
(and no font licences) involved.
