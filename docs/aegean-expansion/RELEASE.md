...
Leonidas has 1,705,050 health and 255 defense. Calibration uses actual level-100 equipment and legal original talent budgets, finite regeneration, bounded projectile counts and estimated attack uptime. It does not apply hidden per-class boss scaling. Restoring original combat and equipment invalidates the first release's quoted class timings; current diagnostics must be read from the rebuilt regression run.
