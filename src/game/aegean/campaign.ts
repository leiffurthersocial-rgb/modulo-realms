...
export class AegeanCampaign {
  ...
  access(id: string): string | null {
    ...
    // Achaea is dangerous, not level-locked. The army can be challenged by
    // anyone who reaches it; only its victory opens the king's storm island.
    if (id === "aegean_leonidas" && !this.has("aegean_army"))
      return "The Three Hundred still hold the king’s island. Defeat the army first.";
  ...
  complete(id: string, repeat = false): void {
    ...
    if (QUEST_BY_ID[id]) g.quests.complete(id);
  ...
  update(dt: number): void {
    ...
    for (const a of AEGEAN_ADVENTURES) {
      if (p.discovered.has(a.id) || g.map.id === a.mapId) {
        p.flags.add(`aegean:known:${a.mapId}`);
        if (this.has(a.mapId)) g.quests.complete(a.mapId);
        else if (!g.quests.isActive(a.mapId)) {
          g.quests.accept(a.mapId);
          if (!g.trackedQuest) g.trackedQuest = a.mapId;
        }
      }
    }
