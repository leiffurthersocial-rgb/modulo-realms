...
export const GREEK_WEAPON_STYLES: Record<string, GreekWeaponStyle> = {
  ...
  aegean_kings_dory: style('royal', 1.0, 160, 'Royal reach', 'Long piercing thrust · heavy follows with a shield-breaking bash'),
  aegean_last_dawn_kopis: style('dawn', 2.05, 70, 'Dawn pursuit', 'Alternating cuts · third attack follows with a returning cut'),
  aegean_storm_cleared_bow: style('storm', .62, 620, 'Storm rail', 'Slow draw · lightning-fast piercing arrow with impact burst'),
  aegean_first_flame_sceptre: style('flame', 1.12, 405, 'First flames', 'Every basic attack cycles lance, fan, then eight flying embers'),
