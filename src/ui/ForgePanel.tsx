...
export default function ForgePanel({ game }: { game: Game }) {
  ...
  return (
    <Modal title={<>{royal ? 'Crown Commission' : 'The Anvil'}</>} sub={<>{royal ? <>{warrants} warrant{warrants === 1 ? '' : 's'} &middot; </> : null}
            {ingots} ingots (iron equivalent) &middot; {ore} ore &middot; {runes} binding runes &middot; {p.gold} gold</>} size="l" tall onClose={() => game.closeAll()}>

        <div className="quest-layout">
          <div className="scroll" style={{ borderRight: '1px solid var(--edge)', overflowY: 'auto', padding: 5 }}>
            {work.length ? (
              <>
            ...
            {all.length === 0 ? (
              <div style={{ color: 'var(--muted)', padding: 5 }}>You carry nothing worth working.</div>
            ) : null}
            {all.map((it) => {
          ...
          <div className="inv-col scroll" style={{ overflowY: 'auto' }}>
            {recipe ? (
              ...
              <div style={{ color: 'var(--muted)' }}>Select a piece of gear.</div>
            )}
          </div>
        </div>
    </Modal>
