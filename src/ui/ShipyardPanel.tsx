      ...
      <div className="modal panel" role="dialog" aria-modal="true" aria-label="Shipwright" style={{ width: 'min(760px, 95vw)', height: 'min(580px, 92vh)' }}>
        ...
        <div className="quest-layout shipyard-layout">
            ...
            {AEGEAN_SHIPS.map((s) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="qi-meta">{vessel ? n.state.selected === s.id ? 'At the dock' : 'Owned' : `${s.cost.toLocaleString()} gold`}</div>
                </button>
              );
            })}
          </div>
          <div className="inv-col scroll" style={{ overflowY: 'auto', minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 16 }}>
              ...
              {owned ? (
                <>
                  ...
                  <button className="btn primary" disabled={!atDock || owned.hull <= 0} onClick={() => { if (selectShip()) n.embark(); }}>Embark</button>
                </>
              ) : null}
            </div>
            {owned ? (
            ...
            <p className="help-p" style={{ marginTop: 16 }}>The shore is safer. Out beyond the shelf, a sound hull and good supplies matter more than speed.</p>
          </div>
        </div>
      </div>
    </div>
