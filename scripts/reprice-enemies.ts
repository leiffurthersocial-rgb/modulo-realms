...
import { enemyDamageAt, enemyHealthAt, enemyXpAt, type EnemyRole } from '../src/data/balance';
...
const entry = /role: '(skirmisher|standard|brute|elite|boss)',([\s\S]{0,400}?)level: (\d+), health: (\d+), damage: (\d+), defense: (\d+), speed: (\d+), xp: (\d+)/g;

let changed = 0;
src = src.replace(entry, (whole, role: string, between: string, lvS: string, hpS: string, dmgS: string, defS: string, spdS: string, xpS: string) => {
  ...
  if (String(hp) === hpS && String(dmg) === dmgS && String(xp) === xpS) return whole;
  changed++;
  console.log(`  lv${level} ${role}: hp ${hpS}->${hp}  dmg ${dmgS}->${dmg}  xp ${xpS}->${xp}`);
  return `role: '${role}',${between}level: ${level}, health: ${hp}, damage: ${dmg}, defense: ${defS}, speed: ${spdS}, xp: ${xp}`;
});
