...
// Only Greek equipment uses the new curve. The exact same roll taken through 99 forge upgrades must equal a fresh curve-priced roll.
const stepped = makeItem("aegean_dory_dawn", {
  level: 1,
  rarity: "legendary",
  rng: new RNG(18971),
});
...
const direct = makeItem("aegean_dory_dawn", {
  level: 100,
  rarity: "legendary",
  rng: new RNG(18971),
});
...
const legacy = makeItem("aegean_dory_dawn", {
  level: 75,
  rarity: "legendary",
  rng: new RNG(7),
});
...
assert.equal(
  legacy.stats.damage,
  scaleStats(TEMPLATE_BY_ID.aegean_dory_dawn, 75, "legendary").damage,
);
assert(
